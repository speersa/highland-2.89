void make_final_plots(TString run5c_data, TString MagnetMC, TString SandMC){
  ...
  TString mom_plot_TitleY[3]={"Events/(100 MeV/c)","Events/(100 MeV/c)","Events/(250 MeV/c)"};
  ...
  for(int ii=0; ii<2; ii++){
	...
	draw->SetTitleX("Muon cos(#theta)");
      ...
      if(ii==0){
	if(ih==2)draw->Draw(exp,Form("%s",variable[ii].Data()),20,0,5000,"all",Form("%s",scuth2[ih].Data()),"","SYS E2");
	else draw->Draw(exp,Form("%s",variable[ii].Data()),25,0,2500,"all",Form("%s",scuth2[ih].Data()),"","SYS E2");
      }
      else draw->Draw(exp,Form("%s",variable[ii].Data()),20,0,1,"all",Form("%s",scuth2[ih].Data()),"","SYS E2");
