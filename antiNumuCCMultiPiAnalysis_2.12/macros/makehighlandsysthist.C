void convert_fname_systname(TString fsystname,TString &systname2,TString &opt){
  ...
  else if(fsystname=="tofres")        {opt="SSYS"; systname2="tof_resol_syst";}
  ...
  else if(fsystname=="fgdhybrideff")  {opt="WSYS"; systname2="fgdhybridtrackeff_syst";}
  ...
  else if(fsystname=="siproton")      {opt="WSYS"; systname2="siproton_syst";}
...
void  DrawRelativeErrors(TH2F *hcov,TH1F* hh,TTree *deftree,TTree *tree,TString var,int nbins,TH1F &hn_nom, TH2F &hn_a, TH2F &hn_va,TString cut_nom,TString opt){
  ...
  if(opt=="WSYS"){
    std::cout<<"WSYS"<<std::endl;
    deftree->Project("hn_nom",Form("%s",var.Data()),Form("(%s)*weight_syst_total",cut_nom.Data()));
    tree->Project("hn_a",Form("%s:toy_index",var.Data()),Form("(%s && weight_syst_total<100)*weight_syst_total",cut_nom.Data()));
    tree->Project("hn_va",Form("%s:toy_index",var.Data()),Form("(%s)*weight_syst_total",cut_nom.Data()));
  }
  else if(opt=="SSYS"){
    std::cout<<"SSYS"<<std::endl;
    deftree->Project("hn_nom",Form("%s",var.Data()),Form("(%s)",cut_nom.Data()));
    tree->Project("hn_a",Form("%s:toy_index",var.Data()),Form("(%s)",cut_nom.Data()));
    tree->Project("hn_va",Form("%s:toy_index",var.Data()),Form("(%s)",cut_nom.Data()));
  }
  else{
...
void makehighlandsysthist(TString stFGD,TString infile,TString outfile,TString update="UPDATE",bool regular=false,int _nbins=50)
{
  ...
  //Asher's binning
  int nbins[4]={5,5,5,5};
  double bins[4][6]={0,500,900,1200,2000,30000,
  ...
  TString systname[19]={"bfield","momresol","momscale","tpcpid","fgdpid","tofres","chargeideff","tpcclustereff","tpctrackeff","fgdhybrideff","tpcfgdmatcheff","micheleleeff","pileup","fgdmass","oofv","sipion","siproton","sandmu","all"};
  ...
  TH1F *hh[4];
  TH2F *hcov[4];
  TString scuth2[4]={"accum_level[][0]>6","accum_level[][0]>7","accum_level[][1]>7","accum_level[][2]>7"};
  TString nn[4]={"CC","CC0pi","CC1pi","CCOth"};  
  TTree *deftree=(TTree*)mc->GetTree("default");
  for(int isyst=0;isyst<19;isyst++){   
