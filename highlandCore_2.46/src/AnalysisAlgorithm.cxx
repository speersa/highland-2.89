...
//********************************************************************
  ...
  // Boolean parameter to know whether any of the methods that sets things into used analysis has been called
  _setMethodCalled=false; 

  //Analysis point
  _analysisPoint=kConstructor;

  // Version check enabled by default
  _versionCheck = true;
  
  // Set to false by default
  _allowNonExclusiveSelections = false;
}
...
//********************************************************************
  ...
  if (_analysisPoint > kInitialize){
    std::cout << "ERROR. AnalysisAlgorithm::UseAnalysis() cannot be called after Initialize() !!!" << std::endl;
    exit(1);
  }
  ...
  for (std::vector<SelectionBase*>::iterator it=sel().GetSelections().begin();it!=sel().GetSelections().end();it++){
    ...
    if (!found){
      ...
      if (!_allowNonExclusiveSelections){ 
        ...
        std::cout << "Set '_allowNonExclusiveSelections=true' in Your selection to allow non-exclusive selections" << std::endl;
        exit(1);
      }
    }
...
//********************************************************************
void AnalysisAlgorithm::InitializeSelection(const SelectionBase& sel){
//********************************************************************
  // Do some initialization given the toy experiment processed
  sel.InitializeToy(_event);
  return;
}

//********************************************************************
void AnalysisAlgorithm::FinalizeSelection(const SelectionBase& sel){
//********************************************************************
  // Finaluze the toy experiment processed
  sel.FinalizeToy(_event);
  return;
}

//********************************************************************
void AnalysisAlgorithm::FillToyVarsInMicroTreesBase(){
//********************************************************************
  // Set the toy dependent event changes to the selection
  sel().GetSelection(_selectedSelection)->InitializeToy(_event);
  
  FillToyVarsInMicroTrees();
  
  sel().GetSelection(_selectedSelection)->FinalizeToy(_event);
