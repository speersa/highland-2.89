...
//********************************************************************
  ...
  // Apply the optimisation by default
  _apply_root_basket_opt = true;
...
//********************************************************************  
  ...
  TTree* tree = GetTree(tree_index);
  
  if (tree){
    
    tree->Fill();
    
    // Make sure that some events are available for optimisation
    if (tree->GetEntries() > 10 && _apply_root_basket_opt){
      
      tree->OptimizeBaskets(10000000); // use the default ROOT value for the moment --> to check
      
      _apply_root_basket_opt = false;
