#ifndef NeutronInteractionSystematic_h
#define NeutronInteractionSystematic_h

#include "SecondaryInteractionSystematic.hxx"
#include "Parameters.hxx"
 
namespace neutron_si{
  
  class NeutronSIPropagator:  public si_syst::ParticleSIPropagator{
    
  public:
    NeutronSIPropagator(){
      // For the moment read the uncertainty from the param file
      _uncertainty = (Float_t)ND::params().GetParameterD("highlandSystematics.NeutronSI.XSec.Uncertainty"); 
    }
  ...
  protected:
     ...
     void SetParameters(){
      ...
      // Set relevant PDGs
      AddParticlePDG(ParticleId::kNeutronPDG);
      
     }
    
  private:
    
    Double_t GetNeutronInelasticCrossSection(const Double_t& momentum, const Double_t& Z, const Double_t& A) const;
    
    Float_t _uncertainty;
   
    static const Float_t _minEnergy;
    static const Float_t _maxEnergy; 
  };  
}
