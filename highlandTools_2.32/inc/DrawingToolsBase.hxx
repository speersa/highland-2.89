...
public :
  ...
  /// Setting it to "" (empty string) will disable adding it to the legend.
  void SetAllMCSystLabel(const std::string& label){_allmcsyst_label=label;}
  ...
  */
  ...
  /// Get all the 2D histos that have been drawn. Useful if you want to save them to file.
  /// NOTE! The "same" plot may appear multiple times due to the way the comparison
  /// histograms are drawn.
  std::vector< TH2* > Get2DHistos() { return _saved_histos2D; }
  ...
  /// Get all the last Histo that was drawn. Useful if you want to modify its appearance.
  TH2* GetLastHisto2D() {
    std::cout << "If it crashes you should maybe use GetLastStackTotal2D instead.\n";
    if (_saved_histos2D.back()) return _saved_histos2D.back();
    else std::cout << "No histos found. You can try GetLastStackTotal2D \n"; return NULL; 
  }
 ...
 protected:
  ...
  /// A function to try substitute some bins of a given histo (e.g. non-valid),  
  /// via the usage of different weights/cuts for the projection: e.g. weight_syst changed to
  /// weight_corr: one has to provide: a tree,  a histo projected from this
  /// tree, variable, original cut used,  a vector of bins to be substituted,  phrase in cut to be
  /// substituted, a new phrase that will be used for substitution     
  void SubstituteBinsWithNewCut(TTree* tree, TH1_h* h, const std::string& var, const std::string& cut, const std::vector<Int_t>& bins, 
      const std::string& phrase_orig, const std::string& phrase_new, bool verbose = true); 
