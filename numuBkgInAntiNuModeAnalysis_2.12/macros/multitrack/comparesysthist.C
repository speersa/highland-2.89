/*This macro allows to compare the results of the systematic propagation for FGD1 and FGD2 sample*/
TString key[2]={"h2","h2"};
TString key2[2]={"_reg20","_reg20"}
TString in_h2_p6t_fgd1="hist_fth2_"+key[0]+"_100toys_6Brun5c_FGD1"+key2[0]+".root";//highland2 FGD1
TString in_h2_p6t_fgd2="hist_fth2_"+key[0]+"_100toys_6Brun5c_FGD2"+key2[0]+".root";//highland2 FGD2
TString inh2sand_fgd1="hist_fth2_"+key[0]+"_100toys_6Brun5cSAND_FGD1"+key2[0]+".root";//sand FGD1
TString inh2sand_fgd2="hist_fth2_"+key[0]+"_100toys_6Brun5cSAND_FGD2"+key2[0]+".root";//sand FGD2
...
gStyle->SetOptStat(1110);
  ...
  for(int iff=0;iff<2;iff++){
    if(iff==0){  
      infsand[0]= new TFile(inh2sand_fgd1.Data(),"READ");//sand MC
      infnom[0] = new TFile(in_h2_p6t_fgd1.Data(),"READ");//magnet MC
    }
    else if(iff==1){  
      infsand[1]= new TFile(inh2sand_fgd2.Data(),"READ");//sand MC
      infnom[1] = new TFile(in_h2_p6t_fgd2.Data(),"READ");//magnet MC
    }    
  }
...
void comparesysthist(int opt){
  makesandmuonsysthist();
  TFile *inf[2];
  inf[0]=new TFile(in_h2_p6t_fgd1.Data(),"READ");//FGD1
  inf[1]=new TFile(in_h2_p6t_fgd2.Data(),"READ");//FGD2 

  std::ofstream txtfile;
  if(opt==0)
    txtfile.open("latex_FGD1.txt");
  else if(opt==1)
    txtfile.open("latex_FGD2.txt");
  ...
  for(int isyst=0;isyst<NSYST;isyst++){
	...
	if(hh[ih][iff])hh[ih][iff]->GetYaxis()->SetRangeUser(0.,vmax*1.1);
      ...
      leg->Draw("same");
      c1[ih]->Print(Form("numubkg/%s_%s.pdf",name.Data(),nn[ih].Data()));
    }
    txtfile<<"\\\\ \n";
  }
  txtfile<<"TOTAL";
  for(int ih=0;ih<3;ih++){
    sprintf(ttt," & %.6f",sqrt(totalerror[ih])*100);
    txtfile<<ttt;
  }
  txtfile<<"\\\\ \n \\hline";
  txtfile.close();
