...
ND::TBasicHeaderModule::TBasicHeaderModule(const TBasicHeaderModule & rhs)
   ...
   , fEventTRef(const_cast<TBasicHeaderModule &>( rhs ).fEventTRef)
   , fEventTime(const_cast<TBasicHeaderModule &>( rhs ).fEventTime)
   , fTriggerWord(const_cast<TBasicHeaderModule &>( rhs ).fTriggerWord)
   , fFGDCosmicEvent(const_cast<TBasicHeaderModule &>( rhs ).fFGDCosmicEvent)
   , fTripTCosmicEvent(const_cast<TBasicHeaderModule &>( rhs ).fTripTCosmicEvent)
   , fTripTCosmicTriggerType(const_cast<TBasicHeaderModule &>( rhs ).fTripTCosmicTriggerType)
   , fTripTCosmicTriggerUpward(const_cast<TBasicHeaderModule &>( rhs ).fTripTCosmicTriggerUpward)
   , fCosmicPrescaleWeight(const_cast<TBasicHeaderModule &>( rhs ).fCosmicPrescaleWeight)
   , fSpill(const_cast<TBasicHeaderModule &>( rhs ).fSpill)
   , fPartition(const_cast<TBasicHeaderModule &>( rhs ).fPartition)
   , fTimeStamp(const_cast<TBasicHeaderModule &>( rhs ).fTimeStamp)
   , fIsDetector(const_cast<TBasicHeaderModule &>( rhs ).fIsDetector)
   , fIsMC(const_cast<TBasicHeaderModule &>( rhs ).fIsMC)
   , fIsSand(const_cast<TBasicHeaderModule &>( rhs ).fIsSand)
   , fP0DWaterStatus(const_cast<TBasicHeaderModule &>( rhs ).fP0DWaterStatus)
   , fMCIntensity(const_cast<TBasicHeaderModule &>( rhs ).fMCIntensity)
   , fMagneticFieldStrength(const_cast<TBasicHeaderModule &>( rhs ).fMagneticFieldStrength)
   , fSoftware(const_cast<TBasicHeaderModule &>( rhs ).fSoftware)
{
   // This is NOT a copy constructor. This is actually a move constructor (for stl container's sake).
   // Use at your own risk!
   if (&rhs) {} // avoid warning about unused parameter
   TBasicHeaderModule &modrhs = const_cast<TBasicHeaderModule &>( rhs );
   modrhs.fEventTRef = 0;
   for (Int_t i=0;i<3;i++) fCTMTriggerPattern[i] = rhs.fCTMTriggerPattern[i];
   for (Int_t i=0;i<5;i++) fGeometryHash[i] = rhs.fGeometryHash[i];
   for (Int_t i=0;i<5;i++) fAlignmentId[i] = rhs.fAlignmentId[i];
   for (Int_t i=0;i<50;i++) fSoftwareVersion[i] = rhs.fSoftwareVersion[i];
}
ND::TBasicHeaderModule::~TBasicHeaderModule() {
   delete fEventTRef;   fEventTRef = 0;
}
...
ND::TP0DReconModule::TP0DReconModule(const TP0DReconModule & rhs)
   ...
   , fNAlgoResults(const_cast<TP0DReconModule &>( rhs ).fNAlgoResults)
   , fNVertices(const_cast<TP0DReconModule &>( rhs ).fNVertices)
   , fNParticles(const_cast<TP0DReconModule &>( rhs ).fNParticles)
   , fNShowers(const_cast<TP0DReconModule &>( rhs ).fNShowers)
   , fNTracks(const_cast<TP0DReconModule &>( rhs ).fNTracks)
   , fNNodes(const_cast<TP0DReconModule &>( rhs ).fNNodes)
   , fNClusters(const_cast<TP0DReconModule &>( rhs ).fNClusters)
   , fNHits(const_cast<TP0DReconModule &>( rhs ).fNHits)
   , fAlgoResults(const_cast<TP0DReconModule &>( rhs ).fAlgoResults)
   , fVertices(const_cast<TP0DReconModule &>( rhs ).fVertices)
   , fParticles(const_cast<TP0DReconModule &>( rhs ).fParticles)
   , fShowers(const_cast<TP0DReconModule &>( rhs ).fShowers)
   , fTracks(const_cast<TP0DReconModule &>( rhs ).fTracks)
   , fNodes(const_cast<TP0DReconModule &>( rhs ).fNodes)
   , fHits(const_cast<TP0DReconModule &>( rhs ).fHits)
   , fClusters(const_cast<TP0DReconModule &>( rhs ).fClusters)
...
ND::TP0DReconModule::~TP0DReconModule() {
   delete fAlgoResults;   fAlgoResults = 0;
   delete fVertices;   fVertices = 0;
   delete fParticles;   fParticles = 0;
   delete fShowers;   fShowers = 0;
   delete fTracks;   fTracks = 0;
   delete fNodes;   fNodes = 0;
...
ND::TP0DECALReconModule::TP0DECALReconModule(const TP0DECALReconModule & rhs)
   ...
   , fNReconTracks(const_cast<TP0DECALReconModule &>( rhs ).fNReconTracks)
   , fNReconShowers(const_cast<TP0DECALReconModule &>( rhs ).fNReconShowers)
   , fNLayerCharge(const_cast<TP0DECALReconModule &>( rhs ).fNLayerCharge)
   , fReconTracks(const_cast<TP0DECALReconModule &>( rhs ).fReconTracks)
   , fReconShowers(const_cast<TP0DECALReconModule &>( rhs ).fReconShowers)
   , fLayerCharge(const_cast<TP0DECALReconModule &>( rhs ).fLayerCharge)
{
   // This is NOT a copy constructor. This is actually a move constructor (for stl container's sake).
   // Use at your own risk!
   if (&rhs) {} // avoid warning about unused parameter
   TP0DECALReconModule &modrhs = const_cast<TP0DECALReconModule &>( rhs );
   modrhs.fReconTracks = 0;
   modrhs.fReconShowers = 0;
   modrhs.fLayerCharge = 0;
}
ND::TP0DECALReconModule::~TP0DECALReconModule() {
   delete fReconTracks;   fReconTracks = 0;
   delete fReconShowers;   fReconShowers = 0;
   delete fLayerCharge;   fLayerCharge = 0;
}
#endif // ND__TP0DECALReconModule_cxx

#ifndef ND__TTrackerECALReconModule__TECALReconVertexCluster_cxx
#define ND__TTrackerECALReconModule__TECALReconVertexCluster_cxx
ND::TTrackerECALReconModule::TECALReconVertexCluster::TECALReconVertexCluster() {
   Tracks = 0;
   Crossings = 0;
}
ND::TTrackerECALReconModule::TECALReconVertexCluster::TECALReconVertexCluster(const TECALReconVertexCluster & rhs)
   : TObject(const_cast<TECALReconVertexCluster &>( rhs ))
   , UniqueID(const_cast<TECALReconVertexCluster &>( rhs ).UniqueID)
   , NHits(const_cast<TECALReconVertexCluster &>( rhs ).NHits)
   , TotalCharge(const_cast<TECALReconVertexCluster &>( rhs ).TotalCharge)
   , Position(const_cast<TECALReconVertexCluster &>( rhs ).Position)
   , Module(const_cast<TECALReconVertexCluster &>( rhs ).Module)
   , MinLayerHit(const_cast<TECALReconVertexCluster &>( rhs ).MinLayerHit)
   , MaxLayerHit(const_cast<TECALReconVertexCluster &>( rhs ).MaxLayerHit)
   , MinParaBarHit(const_cast<TECALReconVertexCluster &>( rhs ).MinParaBarHit)
   , MaxParaBarHit(const_cast<TECALReconVertexCluster &>( rhs ).MaxParaBarHit)
   , MinPerpBarHit(const_cast<TECALReconVertexCluster &>( rhs ).MinPerpBarHit)
   , MaxPerpBarHit(const_cast<TECALReconVertexCluster &>( rhs ).MaxPerpBarHit)
   , N2DTracksMax(const_cast<TECALReconVertexCluster &>( rhs ).N2DTracksMax)
   , N2DTracksMin(const_cast<TECALReconVertexCluster &>( rhs ).N2DTracksMin)
   , G4IDSingle(const_cast<TECALReconVertexCluster &>( rhs ).G4IDSingle)
   , CompletenessSingle(const_cast<TECALReconVertexCluster &>( rhs ).CompletenessSingle)
   , CleanlinessSingle(const_cast<TECALReconVertexCluster &>( rhs ).CleanlinessSingle)
   , G4IDPrimary(const_cast<TECALReconVertexCluster &>( rhs ).G4IDPrimary)
   , CompletenessPrimary(const_cast<TECALReconVertexCluster &>( rhs ).CompletenessPrimary)
   , CleanlinessPrimary(const_cast<TECALReconVertexCluster &>( rhs ).CleanlinessPrimary)
   , G4IDRecursive(const_cast<TECALReconVertexCluster &>( rhs ).G4IDRecursive)
   , CompletenessRecursive(const_cast<TECALReconVertexCluster &>( rhs ).CompletenessRecursive)
   , CleanlinessRecursive(const_cast<TECALReconVertexCluster &>( rhs ).CleanlinessRecursive)
   , NTracks(const_cast<TECALReconVertexCluster &>( rhs ).NTracks)
   , Tracks(const_cast<TECALReconVertexCluster &>( rhs ).Tracks)
   , NCrossings(const_cast<TECALReconVertexCluster &>( rhs ).NCrossings)
   , Crossings(const_cast<TECALReconVertexCluster &>( rhs ).Crossings)
{
   // This is NOT a copy constructor. This is actually a move constructor (for stl container's sake).
   // Use at your own risk!
   if (&rhs) {} // avoid warning about unused parameter
   TECALReconVertexCluster &modrhs = const_cast<TECALReconVertexCluster &>( rhs );
   modrhs.Module.clear();
   modrhs.Tracks = 0;
   modrhs.Crossings = 0;
}
ND::TTrackerECALReconModule::TECALReconVertexCluster::~TECALReconVertexCluster() {
   ...
   delete Crossings;   Crossings = 0;
}
#endif // ND__TTrackerECALReconModule__TECALReconVertexCluster_cxx
...
ND::TTrackerECALReconModule::TECALReconObject::TECALReconObject(const TECALReconObject & rhs)
   ...
   , Radon_XY_First_Prong_NHits(const_cast<TECALReconObject &>( rhs ).Radon_XY_First_Prong_NHits)
   , Radon_XY_First_Prong_Angle(const_cast<TECALReconObject &>( rhs ).Radon_XY_First_Prong_Angle)
   , Radon_XY_First_Prong_Distance(const_cast<TECALReconObject &>( rhs ).Radon_XY_First_Prong_Distance)
   , Radon_XY_Second_Prong_NHits(const_cast<TECALReconObject &>( rhs ).Radon_XY_Second_Prong_NHits)
   , Radon_XY_Second_Prong_Angle(const_cast<TECALReconObject &>( rhs ).Radon_XY_Second_Prong_Angle)
   , Radon_XY_Second_Prong_Distance(const_cast<TECALReconObject &>( rhs ).Radon_XY_Second_Prong_Distance)
   , Radon_XY_Third_Prong_NHits(const_cast<TECALReconObject &>( rhs ).Radon_XY_Third_Prong_NHits)
   , Radon_XY_Third_Prong_Angle(const_cast<TECALReconObject &>( rhs ).Radon_XY_Third_Prong_Angle)
   , Radon_XY_Third_Prong_Distance(const_cast<TECALReconObject &>( rhs ).Radon_XY_Third_Prong_Distance)
   , Radon_XZ_First_Prong_NHits(const_cast<TECALReconObject &>( rhs ).Radon_XZ_First_Prong_NHits)
   , Radon_XZ_First_Prong_Angle(const_cast<TECALReconObject &>( rhs ).Radon_XZ_First_Prong_Angle)
   , Radon_XZ_First_Prong_Distance(const_cast<TECALReconObject &>( rhs ).Radon_XZ_First_Prong_Distance)
   , Radon_XZ_Second_Prong_NHits(const_cast<TECALReconObject &>( rhs ).Radon_XZ_Second_Prong_NHits)
   , Radon_XZ_Second_Prong_Angle(const_cast<TECALReconObject &>( rhs ).Radon_XZ_Second_Prong_Angle)
   , Radon_XZ_Second_Prong_Distance(const_cast<TECALReconObject &>( rhs ).Radon_XZ_Second_Prong_Distance)
   , Radon_XZ_Third_Prong_NHits(const_cast<TECALReconObject &>( rhs ).Radon_XZ_Third_Prong_NHits)
   , Radon_XZ_Third_Prong_Angle(const_cast<TECALReconObject &>( rhs ).Radon_XZ_Third_Prong_Angle)
   , Radon_XZ_Third_Prong_Distance(const_cast<TECALReconObject &>( rhs ).Radon_XZ_Third_Prong_Distance)
   , Radon_YZ_First_Prong_NHits(const_cast<TECALReconObject &>( rhs ).Radon_YZ_First_Prong_NHits)
   , Radon_YZ_First_Prong_Angle(const_cast<TECALReconObject &>( rhs ).Radon_YZ_First_Prong_Angle)
   , Radon_YZ_First_Prong_Distance(const_cast<TECALReconObject &>( rhs ).Radon_YZ_First_Prong_Distance)
   , Radon_YZ_Second_Prong_NHits(const_cast<TECALReconObject &>( rhs ).Radon_YZ_Second_Prong_NHits)
   , Radon_YZ_Second_Prong_Angle(const_cast<TECALReconObject &>( rhs ).Radon_YZ_Second_Prong_Angle)
   , Radon_YZ_Second_Prong_Distance(const_cast<TECALReconObject &>( rhs ).Radon_YZ_Second_Prong_Distance)
   , Radon_YZ_Third_Prong_NHits(const_cast<TECALReconObject &>( rhs ).Radon_YZ_Third_Prong_NHits)
   , Radon_YZ_Third_Prong_Angle(const_cast<TECALReconObject &>( rhs ).Radon_YZ_Third_Prong_Angle)
   , Radon_YZ_Third_Prong_Distance(const_cast<TECALReconObject &>( rhs ).Radon_YZ_Third_Prong_Distance)
   , Radon_3D_First_Prong_Direction(const_cast<TECALReconObject &>( rhs ).Radon_3D_First_Prong_Direction)
   , Radon_3D_First_Prong_NHits(const_cast<TECALReconObject &>( rhs ).Radon_3D_First_Prong_NHits)
   , Radon_3D_Second_Prong_Direction(const_cast<TECALReconObject &>( rhs ).Radon_3D_Second_Prong_Direction)
   , Radon_3D_Second_Prong_NHits(const_cast<TECALReconObject &>( rhs ).Radon_3D_Second_Prong_NHits)
   , Radon_3D_Third_Prong_Direction(const_cast<TECALReconObject &>( rhs ).Radon_3D_Third_Prong_Direction)
   , Radon_3D_Third_Prong_NHits(const_cast<TECALReconObject &>( rhs ).Radon_3D_Third_Prong_NHits)
...
ND::TTrackerECALReconModule::TTrackerECALReconModule(const TTrackerECALReconModule & rhs)
   ...
   , fNReconVertexClusters(const_cast<TTrackerECALReconModule &>( rhs ).fNReconVertexClusters)
   , total(const_cast<TTrackerECALReconModule &>( rhs ).total)
   ...
   , fReconVertexClusters(const_cast<TTrackerECALReconModule &>( rhs ).fReconVertexClusters)
{
   // This is NOT a copy constructor. This is actually a move constructor (for stl container's sake).
   // Use at your own risk!
   if (&rhs) {} // avoid warning about unused parameter
   ...
   modrhs.fReconVertexClusters = 0;
}
ND::TTrackerECALReconModule::~TTrackerECALReconModule() {
   delete fUnmatchedObjects;   fUnmatchedObjects = 0;
   delete fReconVertexClusters;   fReconVertexClusters = 0;
}
...
ND::TGlobalReconModule::TGlobalReconModule(const TGlobalReconModule & rhs)
   ...
   , fNVertices(const_cast<TGlobalReconModule &>( rhs ).fNVertices)
   , fVertices(const_cast<TGlobalReconModule &>( rhs ).fVertices)
   , fPVInd(const_cast<TGlobalReconModule &>( rhs ).fPVInd)
   , fNSVertices(const_cast<TGlobalReconModule &>( rhs ).fNSVertices)
   , fNPIDs(const_cast<TGlobalReconModule &>( rhs ).fNPIDs)
   , fPIDs(const_cast<TGlobalReconModule &>( rhs ).fPIDs)
   , fNTPCOthers(const_cast<TGlobalReconModule &>( rhs ).fNTPCOthers)
   , fTPCOthers(const_cast<TGlobalReconModule &>( rhs ).fTPCOthers)
   , fNTPCUnused(const_cast<TGlobalReconModule &>( rhs ).fNTPCUnused)
   , fNFGD1Unused(const_cast<TGlobalReconModule &>( rhs ).fNFGD1Unused)
   , fNFGD2Unused(const_cast<TGlobalReconModule &>( rhs ).fNFGD2Unused)
   , fNP0DUnused(const_cast<TGlobalReconModule &>( rhs ).fNP0DUnused)
   , fP0DUnused(const_cast<TGlobalReconModule &>( rhs ).fP0DUnused)
   , fSMRDUnused(const_cast<TGlobalReconModule &>( rhs ).fSMRDUnused)
   , fNSMRDTopUnused(const_cast<TGlobalReconModule &>( rhs ).fNSMRDTopUnused)
   ...
   , fNSMRDUnused(const_cast<TGlobalReconModule &>( rhs ).fNSMRDUnused)
   , fP0DOutermostHits(const_cast<TGlobalReconModule &>( rhs ).fP0DOutermostHits)
   , fEarliestTrackMedianHitTime(const_cast<TGlobalReconModule &>( rhs ).fEarliestTrackMedianHitTime)
   , fNFgdTimeBins(const_cast<TGlobalReconModule &>( rhs ).fNFgdTimeBins)
   , fFgdTimeBins(const_cast<TGlobalReconModule &>( rhs ).fFgdTimeBins)
   , fNP0DECAL(const_cast<TGlobalReconModule &>( rhs ).fNP0DECAL)
   , fNDsECAL(const_cast<TGlobalReconModule &>( rhs ).fNDsECAL)
   , fNTrECAL(const_cast<TGlobalReconModule &>( rhs ).fNTrECAL)
   , fNTPC(const_cast<TGlobalReconModule &>( rhs ).fNTPC)
   , fNFGD(const_cast<TGlobalReconModule &>( rhs ).fNFGD)
   , fNP0D(const_cast<TGlobalReconModule &>( rhs ).fNP0D)
   , fNSMRD(const_cast<TGlobalReconModule &>( rhs ).fNSMRD)
   , fNP0DECALIso(const_cast<TGlobalReconModule &>( rhs ).fNP0DECALIso)
   , fNTrECALIso(const_cast<TGlobalReconModule &>( rhs ).fNTrECALIso)
   , fNDsECALIso(const_cast<TGlobalReconModule &>( rhs ).fNDsECALIso)
   , fNTPCIso(const_cast<TGlobalReconModule &>( rhs ).fNTPCIso)
   , fNFGDIso(const_cast<TGlobalReconModule &>( rhs ).fNFGDIso)
   , fNP0DIso(const_cast<TGlobalReconModule &>( rhs ).fNP0DIso)
   , fNSMRDIso(const_cast<TGlobalReconModule &>( rhs ).fNSMRDIso)
{
   // This is NOT a copy constructor. This is actually a move constructor (for stl container's sake).
   // Use at your own risk!
   if (&rhs) {} // avoid warning about unused parameter
   ...
   modrhs.fTPCOthers = 0;
   modrhs.fP0DUnused = 0;
   modrhs.fSMRDUnused = 0;
   modrhs.fFgdTimeBins = 0;
}
ND::TGlobalReconModule::~TGlobalReconModule() {
   delete fTPCPIDs;   fTPCPIDs = 0;
   delete fVertices;   fVertices = 0;
   delete fPIDs;   fPIDs = 0;
   delete fTPCOthers;   fTPCOthers = 0;
   delete fP0DUnused;   fP0DUnused = 0;
   delete fSMRDUnused;   fSMRDUnused = 0;
   delete fFgdTimeBins;   fFgdTimeBins = 0;
}
...
ND::TSmrdReconModule::TSmrdReconModule(const TSmrdReconModule & rhs)
   ...
   , fNSmrdReconHits(const_cast<TSmrdReconModule &>( rhs ).fNSmrdReconHits)
   , fSmrdReconHits(const_cast<TSmrdReconModule &>( rhs ).fSmrdReconHits)
   , fNSmrdIsoTracks(const_cast<TSmrdReconModule &>( rhs ).fNSmrdIsoTracks)
   , fSmrdIsoTracks(const_cast<TSmrdReconModule &>( rhs ).fSmrdIsoTracks)
{
   // This is NOT a copy constructor. This is actually a move constructor (for stl container's sake).
   // Use at your own risk!
   if (&rhs) {} // avoid warning about unused parameter
   TSmrdReconModule &modrhs = const_cast<TSmrdReconModule &>( rhs );
   modrhs.fSmrdReconHits = 0;
   modrhs.fSmrdIsoTracks = 0;
}
ND::TSmrdReconModule::~TSmrdReconModule() {
   delete fSmrdReconHits;   fSmrdReconHits = 0;
   delete fSmrdIsoTracks;   fSmrdIsoTracks = 0;
}
...
ND::TBeamSummaryDataModule::TBeamSummaryDataModule(const TBeamSummaryDataModule & rhs)
   ...
   , fBeamSummaryDataStatus(const_cast<TBeamSummaryDataModule &>( rhs ).fBeamSummaryDataStatus)
   , fND280Spill(const_cast<TBeamSummaryDataModule &>( rhs ).fND280Spill)
   , fBeamSummaryData(const_cast<TBeamSummaryDataModule &>( rhs ).fBeamSummaryData)
   , fTotalEvents(const_cast<TBeamSummaryDataModule &>( rhs ).fTotalEvents)
   , fEventsWithNoBSD(const_cast<TBeamSummaryDataModule &>( rhs ).fEventsWithNoBSD)
{
   // This is NOT a copy constructor. This is actually a move constructor (for stl container's sake).
   // Use at your own risk!
   if (&rhs) {} // avoid warning about unused parameter
   TBeamSummaryDataModule &modrhs = const_cast<TBeamSummaryDataModule &>( rhs );
   modrhs.fBeamSummaryData = 0;
}
ND::TBeamSummaryDataModule::~TBeamSummaryDataModule() {
   delete fBeamSummaryData;   fBeamSummaryData = 0;
}
...
ND::TReconPerformanceEvalModule::TGlobalReconObject::TGlobalReconObject(const TGlobalReconObject & rhs)
   ...
   , MomentumByRange(const_cast<TGlobalReconObject &>( rhs ).MomentumByRange)
   , MomentumByRangeMuon(const_cast<TGlobalReconObject &>( rhs ).MomentumByRangeMuon)
   , MomentumByRangeMuonFlip(const_cast<TGlobalReconObject &>( rhs ).MomentumByRangeMuonFlip)
   , MomentumByRangeElectron(const_cast<TGlobalReconObject &>( rhs ).MomentumByRangeElectron)
   , MomentumByRangeElectronFlip(const_cast<TGlobalReconObject &>( rhs ).MomentumByRangeElectronFlip)
   , MomentumByRangeProton(const_cast<TGlobalReconObject &>( rhs ).MomentumByRangeProton)
   , MomentumByRangeProtonFlip(const_cast<TGlobalReconObject &>( rhs ).MomentumByRangeProtonFlip)
   ...
   , UniqueID(const_cast<TGlobalReconObject &>( rhs ).UniqueID)
{
   // This is NOT a copy constructor. This is actually a move constructor (for stl container's sake).
   // Use at your own risk!
   if (&rhs) {} // avoid warning about unused parameter
...
#ifndef ND__TTRExReconModule_cxx
#define ND__TTRExReconModule_cxx
ND::TTRExReconModule::TTRExReconModule() {
   fPatterns = 0;
}
ND::TTRExReconModule::TTRExReconModule(const TTRExReconModule & rhs)
   : ND::TAnalysisReconModuleBase(const_cast<TTRExReconModule &>( rhs ))
   , fNPatterns(const_cast<TTRExReconModule &>( rhs ).fNPatterns)
   , fIsMC(const_cast<TTRExReconModule &>( rhs ).fIsMC)
   , fPatterns(const_cast<TTRExReconModule &>( rhs ).fPatterns)
   , hackPath(const_cast<TTRExReconModule &>( rhs ).hackPath)
   , hackPatten(const_cast<TTRExReconModule &>( rhs ).hackPatten)
{
   // This is NOT a copy constructor. This is actually a move constructor (for stl container's sake).
   // Use at your own risk!
   if (&rhs) {} // avoid warning about unused parameter
   TTRExReconModule &modrhs = const_cast<TTRExReconModule &>( rhs );
   modrhs.fPatterns = 0;
}
ND::TTRExReconModule::~TTRExReconModule() {
   delete fPatterns;   fPatterns = 0;
}
#endif // ND__TTRExReconModule_cxx
...
ND::TRooTrackerVtxModuleBase::TRooTrackerVtxModuleBase(const TRooTrackerVtxModuleBase & rhs)
   ...
   , fNVtx(const_cast<TRooTrackerVtxModuleBase &>( rhs ).fNVtx)
   , fVtx(const_cast<TRooTrackerVtxModuleBase &>( rhs ).fVtx)
   , fPassThroughPresent(const_cast<TRooTrackerVtxModuleBase &>( rhs ).fPassThroughPresent)
{
   // This is NOT a copy constructor. This is actually a move constructor (for stl container's sake).
   // Use at your own risk!
   if (&rhs) {} // avoid warning about unused parameter
   TRooTrackerVtxModuleBase &modrhs = const_cast<TRooTrackerVtxModuleBase &>( rhs );
   modrhs.fVtx = 0;
}
ND::TRooTrackerVtxModuleBase::~TRooTrackerVtxModuleBase() {
   delete fVtx;   fVtx = 0;
