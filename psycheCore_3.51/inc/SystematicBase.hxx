...
public:
  ...
  const TMatrixD* GetLCovMatrix() const{
    return _L_cov_matrix;
  }
  
  const TMatrixD* GetCorrMatrix() const{
    return _corr_matrix;
  }

  const TMatrixD* GetLCorrMatrix() const{
    return _L_corr_matrix;
  }

  ClassDef(SystematicBase, 3);
...
protected:
  ...
  /// The matrices which may be used for throws
  /// Covariance matrix
  TMatrixD* _cov_matrix; 
  
  /// Correlation matrix
  TMatrixD* _corr_matrix; 

  /// Lower matrix from Cholesky decomposition for covariance
  TMatrixD* _L_cov_matrix; 

  /// Lower matrix from Cholesky decomposition for correlation
  TMatrixD* _L_corr_matrix; 
