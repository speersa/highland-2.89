...
//***********************************************************
  ...
  for (UInt_t j = 0; j < _eventVariations.size(); ++j){
    if (_eventVariations[j]->IsEnabled())
      _eventVariations[j]->InitializeSystBoxes(1,0,1, nevents);
  }
}
...
//***********************************************************
    ...
    // Check that the index is acceptable
    if (index < 0 || index > (Int_t)(NMAXEVENTVARIATIONS - 1)){
      std::cout << "EventVariationManager::AddEventVariation(). '" << index << " is beyond the acceptable range !!!" <<std::endl;        
      exit(1);
    }
...
//***********************************************************
    if (!sys){
        std::cout << "EventVariationManager::ReplaceEventWeight(). '" << index << " sys is not valid !!!" <<std::endl;
        exit(1);
    }
    ...
    for (UInt_t it = 0; it < _eventVariations.size(); it++) {
        if (_eventVariations[it]->GetIndex() == index){
            // for the moment delete the one being replaced, otherwise it wont
            // be deleted 
            ...
            // Set the name if not set before
            if (sys->Name().empty()){
              sys->SetName(_eventVariations[it]->Name());
            }
            
            delete _eventVariations[it];
