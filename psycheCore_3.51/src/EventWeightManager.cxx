...
//***********************************************************
    ...
    if (index < 0 || index > (Int_t)(NMAXEVENTWEIGHTS - 1)){
      std::cout << "EventWeightManager::AddEventWeight(). '" << index << " is beyond the acceptable range !!!" <<std::endl;        
      exit(1);
    }
...
//***********************************************************
    if (!sys){
        std::cout << "EventWeightManager::ReplaceEventWeight(). '" << index << " sys is not valid !!!" <<std::endl;
        exit(1);
    }
    ...
    for (UInt_t it = 0; it < _eventWeights.size(); it++) {
        if (_eventWeights[it]->GetIndex() == index){
            ...
            if (sys->Name().empty()){
              sys->SetName(_eventWeights[it]->Name());
            }
            delete _eventWeights[it];
            _eventWeights[it] = sys; 
            _eventWeightsWithNull[index] = sys;
            
            std::cout << _eventWeights[it]->GetIndex() << std::endl;
            found=true;
            break;
        }
    }
