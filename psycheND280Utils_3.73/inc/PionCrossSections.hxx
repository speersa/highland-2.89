...
namespace PiXSec{
  ...
  template<int nIntBins, int nMomBins>
    bool LoadInfo(Float_t (&xsec_array)[nIntBins][nMomBins], Float_t (&err_array)[nIntBins][nMomBins], bool MC){
      ...
      if(MC) sprintf(inputFileName,"%s/data/PionSI/SIPionXsecMCto8GeV.dat",getenv("PSYCHEND280UTILSROOT"));
      else sprintf(inputFileName,"%s/data/PionSI/SIPionXsecto8GeV.dat",getenv("PSYCHEND280UTILSROOT"));
#else
      if(MC) sprintf(inputFileName,"%s/data/PionSI/SIPionXsecMCto8GeVFixG10.dat",getenv("PSYCHEND280UTILSROOT"));
      else sprintf(inputFileName,"%s/data/PionSI/SIPionXsecto8GeVFixG10.dat",getenv("PSYCHEND280UTILSROOT"));

#endif

      if ((bool)ND::params().GetParameterI("psycheND280Utils.PionSI.UseNEUTCascade")){
        if(MC) sprintf(inputFileName,"%s/data/PionSI/SIPionXsecMCto8GeV_NEUTCascade.dat",getenv("PSYCHEND280UTILSROOT"));
        else sprintf(inputFileName,"%s/data/PionSI/SIPionXsecto8GeV_NEUTCascade.dat",getenv("PSYCHEND280UTILSROOT"));     
      }
      
      if ((bool)ND::params().GetParameterI("psycheND280Utils.PionSI.UseNEUTCascadeProd6TFix")){
        if(MC) sprintf(inputFileName,"%s/data/PionSI/SIPionXsecMCto8GeV_NEUTCascade_p6Tfix.dat",getenv("PSYCHEND280UTILSROOT"));
        else sprintf(inputFileName,"%s/data/PionSI/SIPionXsecto8GeV_NEUTCascade.dat",getenv("PSYCHEND280UTILSROOT"));     
      }
