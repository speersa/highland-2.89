...
//**************************************************
ND280BeamBunching& anaUtils::GetND280Bunching(){
  //**************************************************
  
  return ND280BeamBunching::Instance();  
}
...
Int_t anaUtils::GetRunPeriod(Int_t run, Int_t subrun){
  ...
  else if (run>=10252 && run<10282) return 7;     //run5a FHC
  else if (run>=10282 && run<10303) return 7;     //run5b FHC (run5a code) 
  else if (run>=10334 && run<10518) return 8;     //run5c RHC 
  else if (run==10518 && subrun<=9) return 8;     //run5c RHC  
  else if (run==10518 && subrun>= 28) return 7;   //run5b FHC (run5a code) 
  //
  // run6
  else if (run>=10931 && run<10950) return 13;   //run6a air FHC  
  else if (run>=11305 && run<11329) return 13;   //run6a air FHC
  else if (run>=11436 && run<11443) return 13;   //run6a air FHC 
  else if (run>=10952 && run<11240) return 9;    //run6b air RHC
  else if (run>=11449 && run<11492) return 10;   //run6c air RHC
  else if (run>=11492 && run<11564) return 11;   //run6d air RHC
  else if (run>=11619 && run<11687) return 12;   //run6e air RHC
  else if (run == 11687){
    if(subrun < 7) return 12;                    //run6e  air RHC
    else return 14;                              //run6f  air FHC
  }
  //
  // run7
  ...
  // this is fine because there is no MC (could update run code in future)
  else if (run>=12064 && run<12073) return 16;   //run7a water FHC (run7c code) 
  else if (run>=12080 && run<12557) return 15;   //run7b 
  else if (run>=12563 && run<12590) return 16;   //run7c water
  //
  // run8
  else if (run>=12716 && run<13158) return 17;     //run8 water FHC 
  else if (run>=13190 && run<13738) return 18;     //run8 air   FHC
  //
  // run9
  else if(run>=13888 && run<14032) return 19; //run9b water RHC
  else if(run>=14036 && run<14083) return 20; //run9c water RHC
  else if(run>=14147 && run<14418) return 21; //run9d water RHC
  else if(run>=13846 && run<13879) return 22; //run9a water RHC
  ...
  else if (run>=80600000 && run<=80600159 && (versionUtils::run_numbering_scheme == versionUtils::kPreProd6TNum)) return 9;  //run6b antinu-air
  else if (run>=80600160 && run<=80600219 && (versionUtils::run_numbering_scheme == versionUtils::kPreProd6TNum)) return 10; //run6c antinu-air - have to split Run 6 due to different flux tunings for the different parts
  else if (run>=80600220 && run<=80600299 && (versionUtils::run_numbering_scheme == versionUtils::kPreProd6TNum)) return 11; //run6d antinu-air
  else if (run>=80600300 && run<=80600399 && (versionUtils::run_numbering_scheme == versionUtils::kPreProd6TNum)) return 12; //run6e antinu-air  
  
  // NOTE:  run6 now incompatible with P6T (up to 124) because P6B run6air was done with only 1e17 POT per file
  // while all other runs, and P6T and further, are done with 5e17 POT

  else if (run>=80600000 && run<=80600046) return 9;  //run6b antinu-air
  else if (run>=80600047 && run<=80600065) return 10; //run6c antinu-air - have to split Run 6 due to different flux tunings for the different parts
  else if (run>=80600066 && run<=80600093) return 11; //run6d antinu-air
  else if (run>=80600094 && run<=80600124) return 12; //run6e antinu-air  NOTE:  run6 now incompatible with P6T (up to 124) - need a fix  
 
  else if ( (run>=80730000 && run<=80730999) || (run>=80710000 && run<=80710999) ) return 15; //run7b antinu-water (covers 6L and 6T)
  //else if ( (run>=90830000 && run<=90830013) || (run>=90810000 && run<=90810013) ) return 16; //run7c water        (covers 6L and 6T) - assign fraction of Run8 MC
  else if ( (run>=90830000 && run<=90830999) || (run>=90810000 && run<=90810999) ) return 17; //run8 water         (covers 6L and 6T)
  else if ( (run>=90820000 && run<=90820999) || (run>=90800000 && run<=90800999) ) return 18; //run8 air           (covers 6L and 6T)
  
  else if (  run>=80910000 && run<=80910024) return 20; //run9c antinu-water  (Note:  9a and 9b have no or negligable good POT)  
  else if (  run>=80910025 && run<=80910094) return 21; //run9d antinu-water  - split run9 due to different flux tunings
  ...
  //else if (run>=91810000 && run<=91810013) return 16; //run7c water - assign a fraction of Run8 MC
...
bool anaUtils::IsRHC(int run){
  ...
  // Now check for standard MC and Data
      ...
      GetRunPeriod(run) == 19 ||
      GetRunPeriod(run) == 20 ||
      GetRunPeriod(run) == 21) return true;
...
//*****************************************************************************
bool anaUtils::IsOutOfBunches(const AnaEventB& event, const AnaFgdTimeBinB& bin, const Float_t offset,
      const Float_t lcut, 
      const Float_t rcut){
 //*****************************************************************************  
  
  Float_t binTime = bin.MinTime - offset;

  
  for (UInt_t k = 0 ; k < anaUtils::GetND280Bunching().GetNBunches(event); k++ ){  
    Float_t time =  anaUtils::GetND280Bunching().GetBunchCentralTime(event, k); 
      
    
    if (binTime - time < rcut && binTime - time > -lcut){
      return false;
    }
...
//*****************************************************************************
int anaUtils::GetFGDMichelElectrons(const AnaEventB& event, const SubDetId::SubDetEnum det, AnaFgdTimeBinB** arr){
  //*****************************************************************************  
  ...
  for( Int_t i = 0; i < event.nFgdTimeBins; i++ ){
    ...
    if (!anaUtils::IsOutOfBunches(event, *FgdTimeBin,
          binTimeOffsetToBunch,
          numWidthsReqInSorting * binBunchWidth, 
          numWidthsReqInSorting * binBunchWidth)){
      continue;
    }
...
double anaUtils::GetNTargets(massComponentEnum massComponent, int target_nucleons) {
  ...
  if (target_nucleons == 1) std::cout << "Number of protons: " << nTargets << std::endl; 
  else if (target_nucleons == 2) std::cout << "Number of neutrons: " << nTargets << std::endl;
  else std::cout << "Number of targets: " << nTargets << std::endl;
