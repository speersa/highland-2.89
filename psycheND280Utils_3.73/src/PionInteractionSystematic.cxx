...
PionInteractionSystematic::PionInteractionSystematic(){
  ...
  PiXSec::LoadInfo(PiXSec::xsec_data, PiXSec::err_data, 0);
  PiXSec::LoadInfo(PiXSec::xsec_MC,   PiXSec::err_MC,   1);
...
std::pair<TLorentzVector,TVector3> TakeSmallStep(Int_t charge, 
  ...
  // 24.07.2018: Just use Scinitaller so to cover P0D and ECal as well,  are very similar
  // BUT MAY NEED A REVIEW
  if (materialName.find("Scintillator") == std::string::npos){
...
//FGDGlue: 70
//G10Roha: 80
//Lead: 90 - only exists for the NEUT cascade model

Int_t PionSIManager::GetMaterialSeries(std::string matName) const{
  //Air, use O.
  ...
  // 24.07.2018
  //Iron, use Fe.
  else if (matName == "Iron") return 30;
  //AlRoha2 or AlRoha, use Aluminum.
  else if (matName == "AlRoha" || matName == "AlRoha2") return 20;
  //AlG10
  else if (matName == "AlG10") return 40;
  //WaterSystem
  //29.05.2021 changed the "WaterSystem" to general Oxygen,  see CO2
  else if (matName == "Water") return 10;
  //G10FGD1/G10
  ...
  // 24.07.2018: Just use Scinitaller so to cover P0D and ECal as well,  are very similar
  // BUT MAY NEED A REVIEW
  else if (matName.find("Scintillator") != std::string::npos) return 0;
  //FGDGlue
  ...
  // 24.07.2018: Just use Expoxy so to cover P0DEpoxy as well,  are very similar
  // BUT MAY NEED A REVIEW
  else if (matName == "Epoxy") return 70;
  ...
  // 24.07.2018: Just use Iron so to cover P0D and ECal heavy materials such as 
  // lead and brass,  will be updated when the xsec data is ready for the
  // materials NEED A REVIEW
  else if (matName == "Lead"){ 
    
    // Lead data exist when using NEUT cascade mode
    if (_useNEUTCascade)
      return 90;
    
    return 30;
  }
  else if (matName == "Brass") return 30;
...
Bool_t PionSIManager::InVOI(SubDetId_h det, const Float_t* pos, const Float_t* arr1, const Float_t* arr2) const{  
...
// 4Pi case,  extended FGD1
Bool_t PionSIManager4Pi::InVOI1(const Float_t* pos) const{
  
  //This is from the TPCGasMixture in upstream z through FGD1 all the way out to TPCGasMixture in downstream z.
  //(Rounded to the next integer value that includes the value from the geometry in its range).

  Bool_t xOK = -1150.0 < pos[0] && pos[0] < 1150.0;
  Bool_t yOK = -1170.0 < pos[1] && pos[1] < 1230.0;
  Bool_t zOK =   -12.0 < pos[2] && pos[2] < 576.0;
...
// 4Pi case,  extended FGD1
Bool_t PionSIManager4Pi::InVOIext(const Float_t* pos) const{
  //This is from the TPCGasMixture in upstream z through FGD1 , plus all the way out to TPCGasMixture in TPC3 in downstream z.
  //(Rounded to the next integer value that includes the value from the geometry in its range).
  Bool_t xOK = -1150.0 < pos[0] && pos[0] < 1150.0;
  Bool_t yOK = -1170.0 < pos[1] && pos[1] < 1230.0;
  Bool_t zOK =   -12.0 < pos[2] && pos[2] < 1934.0;
...
//********************************************************************
Bool_t PionSIManager::CheckLookForInteraction(SubDetId_h det, const AnaTrueParticleB& particle, const Float_t* arr1, const Float_t* arr2) const{  
  //********************************************************************
  ...
  return (InVOI(det, particle.PositionEnd, arr1, arr2));
...
SteppingResult PionSIManager::StepBetweenPoints(SubDetId_h det, Int_t charge,
  ...
  while(stepPointSep < initPointSep){
    ...
    if(stepEndMom.Mag() == 0.0 || !InVOI(det, stepEndPosArray, arr1, arr2)){            
...
//##########################################################
PionInteractionSystematic* PionSIManager::ComputePionWeightInfo(const AnaEventB& event, Int_t det, const Float_t* arr1, const Float_t* arr2) const{
  ...
  for (Int_t it = 0; it < nTraj; it++){
    ...
    if(InVOI(det, track->Position, arr1, arr2) ){
      ...
      for (Int_t jt = 0; jt < nTraj; jt++){
        ...
        if(track->ParentID == track2->ID){
          if(InVOI(det, track2->PositionEnd, arr1, arr2) )
            trajOfInterest.push_back(track);
          break;
        }
      }
  ...
  for(UInt_t oai = 0; oai < trajOfInterest.size(); oai++){
    ...
    if (trajOfInterest[oai]->PDG == 211 || trajOfInterest[oai]->PDG == -211){
      if(InVOI(det, trajOfInterest[oai]->Position, arr1, arr2) )
  ...
  for (UInt_t p = 0; p < pionTrajs.size(); p++){
    ...
    if (CheckLookForInteraction(det, *pionTraj, arr1, arr2)){
