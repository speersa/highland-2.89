...
Float_t BeamBunching::GetBunchCentralTime(const AnaEventB& event, Int_t ibunch){
  ...
  if(event.GetIsMC()) {
    if(ibunch > -1 && ibunch < (Int_t)GetNBunches(event))
...
//*****************************************************************************
Int_t BeamBunching::GetBunch(Float_t tTrack, Int_t run, bool isMC, bool cosmic_mode) const {
  //*****************************************************************************
  ...
  for (UInt_t i=0;i<_nbunches;i++){
    ...
    if (tTrack-time < _rcut && tTrack-time > -_lcut){
      bunch = i;
      break;
    }
  }
