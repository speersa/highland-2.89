...
//********************************************************************
Float_t anaUtils::GetNucleusMass(int partPDG ) {
//********************************************************************

  Float_t mass = -1;

  if (partPDG > 1000000000 and partPDG < 2000000000){
    // Nucleon
    // partPDG = 100ZZZAAAI
    // int Z = (partPDG % 1000000000) / 10000;
    int A = (partPDG % 10000) / 10;
    mass = 931.4940954 * A; // approximtion with atomic mass unit
  } else {
...
//********************************************************************
  ...
  } else if (partID == ParticleId::kSigmaPlus) {
    mass = units::mass_sigma_plus;
  } else if (partID == ParticleId::kSigmaMinus) {
    mass = units::mass_sigma_minus;
  } else if (partID == ParticleId::kSigmaZero) {
    mass = units::mass_sigma_zero;
  } else {
    //    std::cout << "Tried to compute mass for invalid particle hypothesis" << std::endl;
    return -1;
  }
